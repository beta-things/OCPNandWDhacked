...
//constants for alarm type and state 
enum 
{   
...
{
public:
	// i think this is a constructor so runs when new allarm is created
    ...
    wxString Options() {
        switch(m_Mode) {
			case TIME: {
				return _("Time") + wxString::Format(_T(" < %f"), m_TimeMinutes);
				break;
			}
			case DISTANCE: {
				return _("Distance") + wxString::Format(_T(" < %f nm"), m_Distance);
				break;
			}
			case ANCHOR: {
    ...
    bool Test() {
		...
		//bool bTriggeredReturn = false;

		//brings in fucntions from ocpnDRAW_PI's ODAPI.cpp 
		...
		//the neccessary evil of obtaining the new API address using the old messaging system? or something about abstrating OD functions to use here
		...
		//kinda is the main show for boundary alarms 
        switch(m_Mode) {
            case TIME: {
				//im not doing shit unless you give me position and vector with speed
                if(wxIsNaN(lastfix.Lat) || wxIsNaN(lastfix.Lon) ||wxIsNaN(lastfix.Cog) || wxIsNaN(lastfix.Sog)) break;
                //also imma be picky about which version you on. do what you gotta do to run this and not the else. 
				//im only making this work with ODAPI and offer no guarantees about JSON in else
				if(ODVersionNewerThan( 1, 1, 1)) {

                    dist = lastfix.Sog * ( m_TimeMinutes / 60 );
					//think the only usefull thing this does is give us coordinates of a point at certain distance off of Bow of boat.
					//returned values &lat and &lon this guy might deal with render for us
                    PositionBearingDistanceMercator_Plugin(lastfix.Lat, lastfix.Lon, lastfix.Cog, dist, &lat, &lon);
                    //again,  so what you gotta do to get into here. fuck the else
					if(l_bUseODAPI_FCBLC) {
						//pFCPIAB should be pFCBLC (chaning from pFCPIAB) but not changing it yet cuz could be the secret sauce that makes this alarm reset when others don't
                        FindClosestBoundaryLineCrossing_t *pFCBLC = new FindClosestBoundaryLineCrossing_t;
						pFCBLC->dStartLat = lastfix.Lat;
						pFCBLC->dStartLon = lastfix.Lon;
						pFCBLC->dEndLat = lat;
						pFCBLC->dEndLon = lon;
                        switch (m_BoundaryType) {
                            case ID_BOUNDARY_ANY:
								pFCBLC->sBoundaryType = "Any";
                                break;
                            case ID_BOUNDARY_EXCLUSION:
								pFCBLC->sBoundaryType = "Exclusion";
                                break;
                            case ID_BOUNDARY_INCLUSION:
								pFCBLC->sBoundaryType = "Inclusion";
                                break;
                            case ID_BOUNDARY_NEITHER:
								pFCBLC->sBoundaryType = "Neither";
                                break;
                            default:
								pFCBLC->sBoundaryType = "Any";
                                break;
                        ...
                        switch (m_BoundaryState) {
                            case ID_BOUNDARY_STATE_ANY:
								pFCBLC->sBoundaryState = "Any";
                                break;
                            case ID_BOUNDARY_STATE_ACTIVE:
								pFCBLC->sBoundaryState = "Active";
                                break;
                            case ID_BOUNDARY_STATE_INACTIVE:
								pFCBLC->sBoundaryState = "Inactive";
                                break;
                        }
                        if((*pODFindClosestBoundaryLineCrossing)(pFCBLC)) {
							if (m_GuardZoneGUID == pFCBLC->sGUID || m_BoundaryGUID == pFCBLC->sGUID) { //make sure we run into the zone we specified i guess probably not how most people want to use is.
								if (m_bCurrentBoatPos)
									m_BoundaryTime = wxTimeSpan();
								else
									m_BoundaryTime = wxTimeSpan::Seconds(3600.0 * dist / lastfix.Sog);

								m_BoundaryAtLat = pFCBLC->dCrossingLat;
								m_BoundaryAtLon = pFCBLC->dCrossingLon;
								m_BoundaryDistance = pFCBLC->dCrossingDistance;
								m_BoundaryName = pFCBLC->sName;
								m_BoundaryDescription = pFCBLC->sDescription;
								m_BoundaryGUID = pFCBLC->sGUID;
								delete pFCBLC;
								return true;
							}
							else {
						...
						wxLogMessage(wxS("l_bUseODAPI_FCBLC not properly set in TIME boundary"));
                        Json::Value jMsg;
                        Json::FastWriter writer;
                        wxString    MsgString;
                        jMsg["Source"] = "WATCHDOG_PI";
				...
				wxLogMessage(wxS("odraw older than 1.1.1. in time boundary"));
            ...
            case DISTANCE: {
                ...
                if( ODVersionNewerThan( 1, 1, 1)) {
					...
					//look around from point in 360 degrees in increments of 10 to find angle at which closest boundary crossing occurs
                    for(double t = 0; t<360; t+=10) {
						//outside function, looks like it generates lat long coordinate points based on last fix position, degree in current for loop and user defined distance
                        PositionBearingDistanceMercator_Plugin(lastfix.Lat, lastfix.Lon, t, m_Distance , &lat, &lon);
						//our ODAPI checks passed, preform task with ODAPI instead of JSON in else
                        if(l_bUseODAPI_FCBLC) {
                            ...
                            if((*pODFindClosestBoundaryLineCrossing)(pFCBLC)) {
                                ...
                                l_BoundaryCrossing.Name = pFCBLC->sName;
                                l_BoundaryCrossing.Description = pFCBLC->sDescription;
                                l_BoundaryCrossing.GUID = pFCBLC->sGUID;
                                l_BoundaryCrossing.Lon = pFCBLC->dCrossingLon;
                                l_BoundaryCrossing.Lat = pFCBLC->dCrossingLat;
                                l_BoundaryCrossing.Len = pFCBLC->dCrossingDistance;
                        ...
                        } //json garbage, dont use
                    } // done 360 look around putting smallest values at top of array for built in comparison for the smallest one
                    // now find the smallest line
					if(!BoundaryCrossingList.empty()) {
                        ...
                        it++;//why the iterator gotta start at 1?
                        while( it != BoundaryCrossingList.end() ) {
                            if( l_dLen > it->Len ) { //if the old length is greater than the new length
                                l_dLen = it->Len;//set our running variable to the new one
								//set all our globals too
                                m_BoundaryName = it->Name;
                                m_BoundaryDescription = it->Description;
                                m_BoundaryGUID = it->GUID;
                                m_BoundaryAtLat = it->Lat; //where the line collision took place
                                m_BoundaryAtLon = it->Lon;
                                m_BoundaryDistance = it->Len;
                            }
                            it++;
                        }
                        return true; //there was some line crossing somewhere so allarm should go
					}
					else { //list was empty, no line crossing no alarm
						return false;
					}
					//for super old version of odraw
                } else {
					wxLogMessage(wxS("odraw older than 1.1.1"));
                    int step_count = 0;
                    for(int count = 11; count > 0; count--) {
                        double t_step;
            ...
            case GUARD: {
				...
				wxLogMessage(wxS("case GUARD ENDTERED"));
				if (wxIsNaN(g_AISTarget.m_dLat) || wxIsNaN(g_AISTarget.m_dLon)) {
					return false;
					break;
				}
				if (l_bUseODAPI_FPIAB) {
					wxLogMessage(wxS("using ODAPI ENDERTERD"));
					FindPointInAnyBoundary_t *pFPIAB = new FindPointInAnyBoundary_t;
					pFPIAB->dLat = g_AISTarget.m_dLat;
					pFPIAB->dLon = g_AISTarget.m_dLon;
					
					wxString d_msg;
					d_msg.append(wxS(" test called from AIS ID:"));
					d_msg.append(g_AISTarget.m_sShipName);
					wxLogMessage(d_msg);
					

					switch (m_BoundaryType) {
						case ID_BOUNDARY_ANY:
							pFPIAB->sBoundaryType = "Any";
							break;
						case ID_BOUNDARY_EXCLUSION:
							pFPIAB->sBoundaryType = "Exclusion";
							break;
						case ID_BOUNDARY_INCLUSION:
							pFPIAB->sBoundaryType = "Inclusion";
							break;
						case ID_BOUNDARY_NEITHER:
							pFPIAB->sBoundaryType = "Neither";
							break;
						default:
							pFPIAB->sBoundaryType = "Any";
							break;
					...
					switch (m_BoundaryState) {
						case ID_BOUNDARY_STATE_ANY:
							pFPIAB->sBoundaryState = "Any";
							break;
						case ID_BOUNDARY_STATE_ACTIVE:
							pFPIAB->sBoundaryState = "Active";
							break;
						case ID_BOUNDARY_STATE_INACTIVE:
							pFPIAB->sBoundaryState = "Inactive";
							break;
					}
		
					//is  our point in any boundary?
					if ((*pOD_FindPointInAnyBoundary)(pFPIAB)) {
						wxLogMessage(wxS("this point is in a boundary"));
						if (m_GuardZoneGUID == pFPIAB->sGUID) { //if the guard zone that the point is in is the one we are checking do the following
							wxLogMessage(wxS("this point is actually in the zone stated in alarm config"));
							
							m_GuardZoneName = pFPIAB->sName;
							m_GuardZoneDescription = pFPIAB->sDescription;
							m_GuardZoneGUID = pFPIAB->sGUID;
						...
						}//point reports being in a boundary but not the one we want (furture question: how would this work for overlapping boundaries?)
						else {
							wxLogMessage(wxS("point in boundary but not the one we care about"));
							delete pFPIAB;
							g_ReceivedGuardZoneMessage = wxEmptyString;
							m_bFired = false;
							return false;
					...
					}//point is not in any boundary
					else {
						wxLogMessage(wxS("point not in any boundary"));
						delete pFPIAB;
						g_ReceivedGuardZoneMessage = wxEmptyString;
						m_bFired = false;
						return false;
				...
				}//  OLD JSON MESSAGING IMPLIMENTED ALONG SIDE ODAPI BELOW BUT IT PISSED ME OFF SO FUCK IT
				else {
	...
	//text populates main readout window (does not control text color...)
    wxString GetStatus() {
        switch(m_Mode) {
            case TIME:
            {
            ...
            {
				if(m_bFired) {
					return " " + wxString(_("Guard Zone")) + ": " + m_GuardZoneName + ": fired";
					
				}
				else {
					return " " + wxString(_("Guard Zone")) + ": " + m_GuardZoneName + ": nothing to report";
				}
				
				/* OLD JSON STUFF--THIS CODE HAS GHOSTS--RUN AWAY AND USE ODAPI
                Json::Value jMsg;
                Json::FastWriter writer;
                wxString    MsgString;
	...
	//create that pretty blinking line un UI to show what alarm is firing
    void Render(wdDC &dc, PlugIn_ViewPort &vp) {
        if(m_bFired) {
            PlugIn_Position_Fix_Ex lastfix = g_watchdog_pi->LastFix();
            wxPoint r1, r2;
	...
	//i think this is specific to setting up boundary allarms only.
    wxWindow *OpenPanel(wxWindow *parent) {
	...
	//run any time the main program decides to give us AIS TARGETS
    void OnAISMessage (int iAlarmIndex) 
    {
        bool l_bFoundMMSI = false;
		//AISMMSITIME is 2 part struct holding MMSI (local unique ID) , and timestamp of last ping recieved?
        AISMMSITIME l_AISMsgInfo;
        
        if(m_bEnabled && g_watchdog_pi->m_WatchdogDialog->IsShown()) { //if the alarm is enabled and the window is open
		   /* std::list<AISMMSITIME>::iterator it = AISMsgInfoList.begin(); //"it" is a vector(array) of AIS objects
            
			while(it != AISMsgInfoList.end()) {//do while we haven't reached the end of this list
                wxFileConfig *l_pConf = GetOCPNConfigObject(); //1_pConf is an object which can read text file("database") info?
                l_pConf->SetPath (  "/Settings/AIS"  ); //would love to be able to find this file..
                
                int l_iLostMins = l_pConf->Read ( "MarkLost_Minutes", 8L );//the fuck does 8L mean?
                
				//if current time minus time revieved target info is greater than iLostMins (the ammount of time we are willing to revive lost targets)
                if((wxDateTime::Now() - it->MsgTime).GetSeconds() > (l_iLostMins * 60)) {
                    //erase this entry because it's older than our lost target timeout
					AISMsgInfoList.erase(it);
					//start at the beggining of the vector
                    it = AISMsgInfoList.begin();
                    continue;
					//else if target UID is the most recently recieved UID from messaging 
                } else if(it->MMSI == g_AISTarget.m_iMMSI) {
					//condition matches we move on with the "it" variable in current state for testing
                    l_bFoundMMSI = true;
                    break;
				}
				else if (it == AISMsgInfoList.end()) {
					break;
				}
				else {
                    it++;
			...
			//somehow we got a tartget  so run a test with it?
            if(Test()) {  
				wxString d_msg;
				d_msg.append(wxS(" alarm hit "));
				d_msg.append(g_AISTarget.m_sShipName);
				wxLogMessage(d_msg);

				wxDateTime now = wxDateTime::Now();

				//keep track of last fire on this allarm and fire if its been longer than our repeat cycle
				if (!alarmFirstFire) {
					if ((now - alarmLastFired).GetSeconds() > m_iRepeatSeconds && m_bRepeat) {
						Run(m_GuardZoneName, m_GuardZoneGUID);
						wxLogMessage(wxS("wait time lapsed. Alarm fired"));
						alarmLastFired = now;
					}
				}
				else {
    ...
    void OnTimer( wxTimerEvent &tEvent )
    {
        switch (m_Mode) {
            ...
            case ANCHOR:
                ...
                break;
            case GUARD:
				//I dont think we want to run this on a timer. It is already called by the AIS targets
              /*Alarm::OnTimer( tEvent );
                if(g_watchdog_pi->m_WatchdogDialog && g_watchdog_pi->m_WatchdogDialog->IsShown())
	...
	//probably pretty low impact, but could be switched over to the new ODAPI
...
//i think this is the entry point.  watchdog_pi.cpp calls us here
void Alarm::LoadConfigAll()
{
	//pulls existing allarms from \ProgramData\opencpn\watchdog\ect...
    wxString configuration = watchdog_pi::StandardPath() + "WatchdogConfiguration.xml";
    TiXmlDocument doc;

	//quick debug check on whether the file exists
...
//triggers sound, command, and messeage box if they are set to do so
void Alarm::Run(wxString alarmName, wxString alarmGUID)

{
    ...
    if(m_bCommand)
        if(!wxProcess::Open(m_sCommand+" "+ alarmGUID)) {
...
//load this allarm's config and populate local values values
    ...
    if(enabled && m_bEnabled) {
        if(Test()) {
            ...
            if(!m_DelayTime.IsValid())
                m_DelayTime = now;
            if((now - m_DelayTime).GetSeconds() >= m_iDelay) {//has it been long enough acording to delay?
                if(m_bFired) {
                    if((now - m_LastAlarmTime).GetSeconds() > m_iRepeatSeconds && m_bRepeat) {//if we've fired, has it been long enough since last alarm acording to irepeatsec
                        Run();
                        m_LastAlarmTime = now;
                    }
                } else {
